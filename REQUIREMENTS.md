...
|-|-|-|-|-|-|
 ...
 | R11 | All tenants will require access to shared services we provide (AD, DNS, NTP) |
 | R12 | Shared services must have a 99.99% uptime |
 | R13 | The security Posture of all workloads must remain consistent even as the move from site to site. This must happen without any additional administrative overhead |
 | R14 | We need to provide tenants in both our shared and dedicated IaaS solution access to enhanced security services if desired. |
 | R15 | The solution needs to scale beyond 1500 tenants with 5% or less of those tenants using our dedicated IaaS solution |
 | R16 | Due to security requirements, dedicated tenants cannot share T0 gateways with other tenants |
 | R17 | Dedicated IaaS tenants are provided access to both our VDI solution and site-to-site VPN as a value-added service |
 | R18 | Tenants must be isolated from one another. One tenant should not be allowed access to other tenants |
 | R19 | All tenants must have access to shared services provided by ZCS |
 | R20 | The solution must minimize administrative overhead |
 | R21 | Route-based VPN must be used for site-to-site VPN configuration from tenants’ dedicated IaaS and their on-premise datacenter |
 | R22 | All Tenant applications need communication with Shared services provided by ZCS (AD, DNS, and NTP services, etc…). These services are running in the Physical infrastructure on bare metal servers. |
 | R23 | Tenants should have external access to their applications on port 443. |
 ...
 | R25 | From the Shared VDI environment, admin users should have HTTP, HTTPS, SSH RDP, and VNC access to their virtual machines. |
 ...
 | R27 | With potentially multiple virtual routers the solution should minimize the need to update BGP neighbor configuration every time a new T0 gateway is deployed |
